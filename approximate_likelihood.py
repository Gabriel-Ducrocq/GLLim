...
class likelihood_approximation:
    def __init__(self, lmax=config.lmax, nside=config.nside, noise_covar_temp=config.noise_covar_temp,
        ...
        self.l_cut = l_cut
        self.bl_gauss = hp.gauss_beam(fwhm=fwhm_radians, lmax=lmax)
    ...
    def generate_data(self, N):
        ...
        for i, theta in enumerate(all_theta):
            ...
            for l in range(2, self.l_cut+1):
                ...
                number_modes = 10000000

                slms = np.random.normal(scale = np.sqrt(1/2), size = (2, number_modes)) \
                + 1j * np.random.normal(scale = np.sqrt(1/2), size = (2, number_modes))
                slms = np.dot(sqrt_cov, slms)

                nlms = np.random.normal(scale = np.sqrt(1/2), size = (2, number_modes)) \
                + 1j * np.random.normal(scale = np.sqrt(1/2), size = (2, number_modes))
                nlms = np.sqrt(self.bl_gauss[l]*self.noise_covar) * nlms

                alms = slms + nlms

                all_cls_tt_hat[i, l-2] = np.mean(np.abs(alms[0, :])**2)
                all_cls_ee_hat[i, l-2] = np.mean(np.abs(alms[1, :])**2)
                all_cls_te_hat[i, l-2] = 2*np.sum((alms[0, :].real*alms[1, :].real + alms[0, :].imag*alms[1, :].imag))/number_modes
            ...
            end_again = time()
            plt.plot(all_cls_tt_hat[i, :])
            plt.plot(cls_tt[2:], color="red")
            plt.show()
            print("Total time:", end_again- start)
        return all_cls_tt_hat, all_cls_ee_hat, all_cls_te_hat
